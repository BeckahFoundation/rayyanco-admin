                  ...
                  {orderItems.map((item: any) => (
                    <div key={item.id} className="border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                      <div className="flex justify-between items-start">
                        <div>
                          <Link href={`/orders/${item.order?.id ?? ''}`} className="text-sm font-medium text-orange-600 hover:underline">
                            #{item.order?.order_number ?? '—'}
                          </Link>
                          <p className="text-xs text-gray-500 mt-0.5">{item.order?.customer_name}</p>
                        </div>
                      ...
                      <div className="flex justify-between mt-1">
                        <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${
                          ...
                          item.order?.status === 'cancelled' ? 'bg-red-100 text-red-600' :
